...
# Исходные параметры
x1, x2, dx = -1000, 1000, 20
y1, y2, dy = -1000, 1000, 20
H = 0
hd = 0
...
xy = cp.arange(x1, x2 + dx, dx, dtype='float32')

x_shape = ((x2-x1) // dx) + 1
y_shape = ((y2-y1) // dy) + 1
x = cp.tile(xy, x_shape)
y = cp.repeat(xy, y_shape)
h = cp.zeros(x.shape[0], dtype='float32')

# Преобразуем в массив точек (Nx2)
xyHh = cp.column_stack((X.ravel(), Y.ravel()))
#xyHh = cp.array([[0, 0], [100, 100]])
...
max_len_calc = 80_000_000

X = cp.arange(x_start, x_start + dx_obj*(shape_cube[0]+1), dx_obj, dtype='float32')
Y = cp.arange(y_start, y_start + dy_obj*(shape_cube[1]+1), dy_obj, dtype='float32')
Z = cp.arange(z_start, z_start + dz_obj*(shape_cube[2]+1), dz_obj, dtype='float32')
...
dT = dT_Eff_M(Tnorm, xyHh[:, :3], xyHh[:, 3], xyzMod, dxyz, Kappa, Q, Dr, Ir, len_calc, max_len_calc, shape_cube, x, y, h, X, Y, Z)
