...
def calc(tic, s, t, u, s1, s2, t1, t2, u1, u2, d, eps, n_pt, n_objs, batches, shape):
    ...
    sqq = as_strided(sq, shape=(batches, shape[0]+1, (shape[1]+1) * (shape[2]+1)), strides=(st_s[0], 4, 0)).flatten()
    tqq = as_strided(tq, shape=(batches, shape[1]+1, shape[2]+1), strides=(st_t[0], 4, 0)).flatten()
    tqq2 = as_strided(tqq, shape=(batches, shape[0]+1, (shape[1]+1) * (shape[2]+1)), strides=(st_t[0]*(shape[2]+1), 0, 4)).flatten()
    uqq = as_strided(uq, shape=(batches, (shape[0]+1) * (shape[1]+1), shape[2]+1), strides=(st_u[0], 0, 4)).flatten()

    q = cp.column_stack((sqq, tqq2, uqq))
    sums = cp.sum(q, axis=1)
    r = cp.sqrt(sums)

    shape_vertex = (shape[0]+1)*(shape[1]+1)*(shape[2]+1)
    bytes_cube = shape_vertex*4
    num_nodes = cp.arange(0, shape_vertex*batches)

    offset_x = (shape[1]+1) * (shape[2]+1)
    offset_y = shape[2]+1

    _111 = as_strided(num_nodes, shape=(shape[0], shape[1], shape[2], batches), strides=(offset_x*4, offset_y*4, 4, bytes_cube)).flatten()
    _211 = _111 + offset_x
    _121 = _111 + offset_y
    _221 = _121 + offset_x
    _112 = _111 + 1
