...
def dT_Eff_M(Tnorm, xyh, hd, xyzCL, dxyz, Kappa, Q, Dr, Ir, L, ML, shape, X, Y, Z):
    ...
    dT = dTVrr(xyh, xyzCL[:, :3], dxyz, n_pt, n_objs, d, drmin, Kappa, L, ML, shape, X, Y, Z)
...
def dTVrr(xyh, xyz, dxyz, n_pt, n_objs, d, eps, Kappa, L, ML, shape, X, Y, Z):
    ...
    if batches != 0:
        ...
        dT = calc(s, t, u, d, eps, n_pt, n_objs, n_pt, shape)

    return dT * Kappa

def calc(s, t, u, d, eps, n_pt, n_objs, n_chunks, shape):
    ...
    #_111 = as_strided(num_nodes_2, shape=(shape[0], shape[1], shape[2]*n_chunks), strides=(n_chunks*offset_x*4, n_chunks*offset_y*4, 4))
    #_1111 = _111.flatten()

    i = cp.arange(shape[0], dtype=cp.int32).reshape(-1, 1, 1)
    j = cp.arange(shape[1], dtype=cp.int32).reshape(1, -1, 1)
    k = cp.arange(shape[2] * n_chunks, dtype=cp.int32).reshape(1, 1, -1)

    idx = i * n_chunks * offset_x + j * n_chunks * offset_y + k

    result = num_nodes_2[idx]
    res_1 = result.flatten()
    ...
    #Psqq = as_strided(sq, shape=(n_chunks, shape[0]+1, (shape[1]+1) * (shape[2]+1)), strides=(st_s[0], 4, 0)).flatten()
    #Ptqq_ = as_strided(tq, shape=(n_chunks, shape[1]+1, shape[2]+1), strides=(st_t[0], 4, 0)).flatten()
    #Ptqq = as_strided(Ptqq_, shape=(n_chunks, shape[0]+1, (shape[1]+1) * (shape[2]+1)), strides=(st_t[0]*(shape[2]+1), 0, 4)).flatten()
    #Puqq = as_strided(uq, shape=(n_chunks, (shape[0]+1) * (shape[1]+1), shape[2]+1), strides=(st_u[0], 0, 4)).flatten()

    # sqq
    sqq = cp.repeat(sq[:, :, None], (shape[1] + 1) * (shape[2] + 1), axis=2).flatten()
    # tqq_
    tqq_ = cp.repeat(tq[:, :, None], shape[2] + 1, axis=2).flatten()
    # tqq
    tqq_reshaped = tqq_.reshape(n_chunks, shape[1] + 1, shape[2] + 1)
    tqq = cp.repeat(tqq_reshaped[:, None, :, :], shape[0] + 1, axis=1)
    tqq = tqq.reshape(n_chunks, (shape[0] + 1) * (shape[1] + 1) * (shape[2] + 1)).flatten()
    # uqq
    uqq = cp.repeat(uq[:, None, :], (shape[0] + 1) * (shape[1] + 1), axis=1).flatten()

    q = cp.column_stack((sqq, tqq, uqq))

    sums = cp.sum(q, axis=1)
